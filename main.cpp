...
int main(int argc, char *argv[])
{
    try
    {
        ...
        auto blk = bulk::BulkContext(commandsCount,
                                     &console_queue, &file_queue,
                                     &console_mutex, &file_mutex,
                                     &console_cv, &file_cv);
...
//        auto fdt2 = async(std::launch::async, fileDumper, ref(file_queue), ref(file2_metr));
        ...
        thread cdt (consoleDumper, ref(console_queue), ref(log_metr)),
               fdt1 (fileDumper, ref(file_queue), ref(file1_metr)),
               fdt2 (fileDumper, ref(file_queue), ref(file2_metr));
